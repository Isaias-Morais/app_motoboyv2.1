...
def adicionar_moto(moto:MotoCreate,motoboy:Motoboy = Depends(get_current_user_id) ,db:Session = Depends(get_db)):
   return registra_moto(moto=moto,session=db,motoboy_id=motoboy)

@router.get('/motos')
async def buscar_motos_motoboy(motoboy_id:int = Depends(get_current_user_id), db:Session=Depends(get_db)):
