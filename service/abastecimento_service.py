...
from routers.moto_router import buscar_motoa
...
def registra_abastecimento(abastecimento:AbastecimentoCreate,session:Session,motoboy_id:int):
    ...
    moto:Moto = busca_moto_ativa_service(session=session,motoboy_id=moto.id)

    novo_abastecimento = Abastecimento(
            data_abastevimento=data,
    ...
    if not abastecimento_existe(session,abastecimento.moto_id,abastecimento.quilometragem_abastecimento):
        salvar_objeto(session,abastecimento)
    else:
        return 'abastecimento ja existente'

    if not abastecimento.tanque_completo:
        return None

    historico = busca_abastecimento_consumo_medio(session,abastecimento.moto_id)
    ...
    atualizar_consumo_moto(session=session,moto_id=abastecimento.moto_id,consumo=consumo_medio)

    km_atual = quilometragem_atual(session, abastecimento.moto_id)

    valido, erro = validar_quilometragem_nova(km_atual, abastecimento.quilometragem_abastecimento)
    if valido:
        atualizar_quilometragem(session, abastecimento.moto_id, abastecimento.quilometragem_abastecimento)
    else:
        return False
